      ...
      <div className="flex flex-col">
        <div className="flex space-y-1 my-8 flex-col leading-[28px] lg:leading-[40px] align-top body-text text-[30px] lg:text-[50px] text-green">
          <p className="body-text text-green">
            There&apos;s a lot to choose from in Nashville. Coffee shops are no
            exception. No one wants to commit the faux pas of showing up at a
            social shop to do work. Yikes. Let&apos;s find your atmosphere.
          </p>
        </div>
      </div>
      <Button
...
const ShowShops = ({ selectedCats, shops, reset }) => {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex space-y-1 my-8 mx-4 lg:mx-10 flex-col leading-[28px] lg:leading-[40px] align-top body-text text-[30px] lg:text-[50px] text-green">
        <p className="body-text text-green">Your atmosphere is...</p>
        {selectedCats.map((cat) => (
          <p
            className="body-text text-green text-center p-3 w-auto rounded-full bg-pink"
            key={cat}
          >
            {cat}
          </p>
...
export default function TestUI(props) {
  ...
  return (
    <div className="mb-10">
      ...
      {step === 2 ? (
        <ShowShops selectedCats={selectedCats} shops={shops} reset={reset} />
      ) : null}
    </div>
  );
}
